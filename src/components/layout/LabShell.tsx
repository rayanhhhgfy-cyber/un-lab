...
export default function LabShell({
  ...
  const isModelApproximationHub = backHref === "/biology" || backHref === "/earth-science";
    ...
    >
      ...
      <div className="relative z-10 pt-20 sm:pt-24 md:pt-28 pb-16 sm:pb-20 px-3 sm:px-6 md:px-10 lg:px-14">
        <div className="max-w-[1600px] mx-auto">
          ...
          {isModelApproximationHub && (
            <motion.div
              initial={{ opacity: 0, y: 8 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.35, delay: 0.1 }}
              className="mb-6 sm:mb-8 rounded-2xl border border-amber-400/25 bg-amber-500/[0.07] px-4 sm:px-5 py-3.5 sm:py-4"
            >
              <p className="text-[11px] sm:text-xs font-semibold text-amber-200/90 leading-relaxed">
                {isArabic
                  ? "ملاحظة صلاحية النموذج: هذه النتائج تعتمد على نماذج تعليمية مبسطة لتوضيح المفاهيم العلمية. القيم المعروضة تقريبية وليست بديلاً عن النماذج البحثية أو القياسات المخبرية المعتمدة، ولا ينبغي استخدامها لاتخاذ قرار علمي أو طبي أو هندسي."
                  : "Model validity note: these outputs are educational simplified models for concept learning. Displayed values are approximate and should not be treated as a substitute for research-grade simulations or laboratory measurements, and must not be used for scientific, medical, or engineering decisions."}
              </p>
            </motion.div>
          )}
