...
function GlassShape({
  ...
  useFrame((_state, delta) => {
    ...
    // gentle scroll-driven Y offset + rotation - throttled for performance
    meshRef.current.position.y = baseY + scrollY * scrollInfluence * 3;
    meshRef.current.rotation.y += delta * 0.1; // Reduced rotation speed
    meshRef.current.rotation.x += delta * 0.05; // Reduced rotation speed
  });
    ...
    >
      ...
      >
        <MeshTransmissionMaterial
          backside={false}
          resolution={128} // Further reduced for performance
          samples={1}      // Keep at 1 for performance
          thickness={0.2}
          chromaticAberration={0.02} // Reduced chromatic aberration
          anisotropy={0}
          distortion={0.05} // Reduced distortion
          distortionScale={0.05} // Reduced distortion scale
          temporalDistortion={0.02} // Reduced temporal distortion
          ior={1.2}
          color={color}
          transmission={0.9}
          roughness={0.1}
...
export default function AntiGravityModel() {
  ...
  // Pre-create geometries once - optimized for performance
  const geometries = useMemo(
    () => ({
      icosahedron: new THREE.IcosahedronGeometry(1, 0), // Reduced detail level
      octahedron: new THREE.OctahedronGeometry(1, 0),
      dodecahedron: new THREE.DodecahedronGeometry(1, 0),
      torusKnot: new THREE.TorusKnotGeometry(0.7, 0.25, 64, 8), // Reduced segments
      sphere: new THREE.SphereGeometry(1, 16, 16), // Reduced segments
    }),
    []
  );
