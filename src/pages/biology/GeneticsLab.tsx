...
function MendelianCross() {
  ...
  const cross = () => {
    const a1 = parentA[0], a2 = parentA[1]; const b1 = parentB[0], b2 = parentB[1];
    const results = [`${a1}${b1}`,`${a1}${b2}`,`${a2}${b1}`,`${a2}${b2}`];
    setOffspring(results);
  };
  useEffect(()=>{cross();},[parentA,parentB]);
  const phenotype = (g:string)=>g.includes('A')?'Dominant':'Recessive';
  const domCount = offspring.filter(g=>g.includes('A')).length;
