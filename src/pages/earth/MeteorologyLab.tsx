...
function StormDynamicsSim() {
  ...
  const windSpeed = stormWindSpeed(pressure, temp);
  const rainChance = stormRainChance(humidity);
...
function GreenhouseSim() {
  ...
  const surfaceTemp = greenhouseSurfaceTemp(co2, albedo);
  const seaLevelRise = estimatedSeaLevelRise(surfaceTemp);
...
export default function MeteorologyLab() {
  ...
  const validityRanges: Record<string, { en: string; ar: string }> = {
    storm: {
      en: "Educational range: pressure 940-1050 hPa, temperature -10 to 45 C, humidity 0-100%. Outputs are qualitative trend indicators, not operational forecasts.",
      ar: "نطاق تعليمي: الضغط 940-1050 هكتوباسكال، الحرارة -10 إلى 45 م، الرطوبة 0-100%. النتائج مؤشرات اتجاهية وليست تنبؤًا تشغيليًا معتمدًا.",
    },
    coriolis: {
      en: "Educational range: latitude 0-90 degrees and object speed 10-500 m/s. This model illustrates direction and scaling only, not full atmospheric dynamics.",
      ar: "نطاق تعليمي: خط العرض 0-90 درجة وسرعة الجسم 10-500 م/ث. هذا النموذج يوضح الاتجاه ومقياس التأثير فقط، وليس ديناميكا الغلاف الجوي الكاملة.",
    },
    greenhouse: {
      en: "Educational range: CO2 280-1000 ppm and albedo 10-60%. Temperature and sea-level outputs are simplified sensitivity estimates, not climate-projection products.",
      ar: "نطاق تعليمي: ثاني أكسيد الكربون 280-1000 جزء بالمليون والبياض 10-60%. مخرجات الحرارة ومستوى البحر تقديرات حساسية مبسطة وليست إسقاطات مناخية بحثية.",
    },
  };
  return (
    <LabShell
