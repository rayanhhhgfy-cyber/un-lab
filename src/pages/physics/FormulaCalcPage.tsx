...
const formulas = {
  en: [
    ...
    { name: "Ohm's Law (V)", formula: "V = IR", inputs: ["current (A)", "resistance (Ω)"], calc: (v: number[]) => v[0] * v[1], result: "V" },
    {
      ...
      calc: (v: number[]) => currentFromVoltage(v[0], v[1]) ?? Number.NaN,
      result: "A",
      validate: (v: number[]) => v[1] !== 0,
    },
    { name: "Force", formula: "F = ma", inputs: ["mass (kg)", "acceleration (m/s²)"], calc: (v: number[]) => v[0] * v[1], result: "N" },
    {
      name: "Ideal Gas (P)",
      formula: "P = nRT/V",
      inputs: ["moles", "temperature (K)", "volume (L)"],
      calc: (v: number[]) => idealGasPressurePa(v[0], v[1], v[2]) ?? Number.NaN,
      result: "Pa",
      validate: (v: number[]) => v[0] >= 0 && v[1] > 0 && v[2] > 0,
    },
    { name: "Wave Speed", formula: "v = fλ", inputs: ["frequency (Hz)", "wavelength (m)"], calc: (v: number[]) => v[0] * v[1], result: "m/s" },
    {
      name: "Coulomb Force",
      formula: "F = kq₁q₂/r²",
      inputs: ["q1 (C)", "q2 (C)", "distance (m)"],
      calc: (v: number[]) => coulombForce(v[0], v[1], v[2]) ?? Number.NaN,
      result: "N",
      validate: (v: number[]) => v[2] > 0,
    },
    { name: "Photon Energy", formula: "E = hf", inputs: ["frequency (Hz)"], calc: (v: number[]) => SCIENCE_CONSTANTS.PLANCK_J_S * v[0], result: "J" },
    {
      name: "de Broglie λ",
      formula: "λ = h/p",
      inputs: ["momentum (kg·m/s)"],
      calc: (v: number[]) => deBroglieWavelength(v[0]) ?? Number.NaN,
      result: "m",
    ...
    },
    {
      ...
      formula: "γ = 1/√(1-v²/c²)",
      inputs: ["velocity (m/s)"],
      calc: (v: number[]) => lorentzFactor(v[0]) ?? Number.NaN,
      result: "",
      validate: (v: number[]) => v[0] >= 0 && v[0] < SCIENCE_CONSTANTS.SPEED_OF_LIGHT_M_PER_S,
    },
    { name: "Centripetal Force", formula: "F = mv²/r", inputs: ["mass (kg)", "velocity (m/s)", "radius (m)"], calc: (v: number[]) => v[0] * v[1] * v[1] / v[2], result: "N" },
    {
      name: "Gravitational Force",
      formula: "F = Gm₁m₂/r²",
      inputs: ["m1 (kg)", "m2 (kg)", "distance (m)"],
      calc: (v: number[]) => SCIENCE_CONSTANTS.GRAVITATIONAL_CONSTANT_M3_PER_KG_S2 * v[0] * v[1] / (v[2] * v[2]),
      result: "N",
      validate: (v: number[]) => v[2] > 0,
    },
    {
      ...
      calc: (v: number[]) => escapeVelocity(v[0], v[1]) ?? Number.NaN,
      result: "m/s",
      validate: (v: number[]) => v[0] >= 0 && v[1] > 0,
    },
  ],
  ar: [
    ...
    {
      name: "الغاز المثالي (ض)",
      formula: "P = nRT/V",
      inputs: ["المولات", "الحرارة (ك)", "الحجم (ل)"],
      calc: (v: number[]) => idealGasPressurePa(v[0], v[1], v[2]) ?? Number.NaN,
      result: "باسكال",
      validate: (v: number[]) => v[0] >= 0 && v[1] > 0 && v[2] > 0,
    },
    { name: "سرعة الموجة", formula: "v = fλ", inputs: ["التردد (هرتز)", "الطول الموجي (م)"], calc: (v: number[]) => v[0] * v[1], result: "م/ث" },
    {
      name: "قوة كولوم",
      formula: "F = kq₁q₂/r²",
      inputs: ["q1 (ك)", "q2 (ك)", "المسافة (م)"],
      calc: (v: number[]) => coulombForce(v[0], v[1], v[2]) ?? Number.NaN,
      result: "نيوتن",
      validate: (v: number[]) => v[2] > 0,
    },
    { name: "طاقة الفوتون", formula: "E = hf", inputs: ["التردد (هرتز)"], calc: (v: number[]) => SCIENCE_CONSTANTS.PLANCK_J_S * v[0], result: "جول" },
    {
      name: "طول دي بروجلي",
      formula: "λ = h/p",
      inputs: ["الزخم (كغ·م/ث)"],
...
export default function FormulaCalcPage() {
  ...
  const calculate = () => {
    ...
    if (selected.validate && !selected.validate(nums)) {
      setAnswer(
        isArabic
          ? "خارج نطاق صلاحية النموذج"
          : "Out of model-valid range",
      );
      return;
    }
    try {
