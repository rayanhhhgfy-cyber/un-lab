...
export default defineConfig(() => ({
  ...
  plugins: [
    ...
    VitePWA({
      ...
      workbox: {
        // Precache the full app shell so every page loads offline.
        // Workbox stores each entry with a content-based revision so stale
        // chunks are replaced atomically and old caches are cleaned up.
        globPatterns: [
          "**/*.{js,css,html,ico,png,svg,webp,woff,woff2,webmanifest}",
        ],
        maximumFileSizeToCacheInBytes: 8 * 1024 * 1024,
        navigateFallback: "/index.html",
        // Don't intercept API or OG-image edge function routes offline.
        navigateFallbackDenylist: [/^\/api\//, /^\/og\.png$/],
        cleanupOutdatedCaches: true,
        skipWaiting: true,
        clientsClaim: true,
        runtimeCaching: [
          {
            // Google Fonts CSS (stale-while-revalidate so it stays fresh)
          ...
          {
            // Google Fonts WOFF/WOFF2 files (immutable — cache-first)
          ...
          {
            // External images / CDN assets — network first with cache fallback
    ...
    rollupOptions: {
      output: {
        manualChunks(id: string) {
          ...
          // React core + every small package that calls hooks at module init
          // must live in ONE chunk so React is always defined when they load.
          if (
